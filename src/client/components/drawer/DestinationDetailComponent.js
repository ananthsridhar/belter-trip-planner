...
function DestinationDetail(props) {
  ...
  let destination = props.destinations.find((dest) => dest.id == props.destId);
  let widgets;
    ...
    <div style={{ textAlign: "center" }}>
      ...
      <Grid container spacing={3}>
        {widgets &&
          ...
          <Grid item xs={12}>
            <Button color="secondary" variant="outlined" onClick={() => removeDest()}>Delete Destination</Button>
          </Grid>
      </Grid>
    </div>
  );
...
export default connect(
  mapStateToProps, { removeDestination })(DestinationDetail);
